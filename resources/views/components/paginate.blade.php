  ...
  <ul class="pagination">
    ...
    @else
      <li class="page-item first" style="opacity: 0.5;">
        <a class="page-link" href="javascript:void(0);" style="cursor: not-allowed;"><i
    ...
    @foreach (range(max(1, $paginator->currentPage() - 2), min($paginator->lastPage(), $paginator->currentPage() + 2)) as $page)
      <li class="page-item{{ $page == $paginator->currentPage() ? ' active' : '' }}">
        <a class="page-link"
          href="{{ $page == $paginator->currentPage() ? 'javascript:void(0);' : $paginator->url($page) }}">{{ $page }}</a>
      </li>
    @endforeach
    {{-- Pages after current page --}}
    @if ($paginator->currentPage() < $paginator->lastPage() - 2)
      <span class="align-self-center ms-1">...</span>
